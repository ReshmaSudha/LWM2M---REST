...
public class LWM2MClientImpl {
	...
	@POST
	@Produces("application/json")
	public Response updateProductionRate(@PathParam("EndPointClientName") String endPntCltNm, String PackagingRate) {
		OutputStreamWriter outputStreamWriter = null;
        InputStream inputStream = null;
        BufferedReader bufferedReader = null;
        try {
        	String status = new JSONObject(PackagingRate).getJSONObject("PRODRATEUPDATEREQUEST").getString("STATUS").toString();
			if(!status.equals("STOPPED")){
				...
				httpURLConnection.setRequestMethod("POST");
				outputStreamWriter = new OutputStreamWriter(httpURLConnection.getOutputStream());
				System.out.println("Update manufaturing rate request from client - "+ endPntCltNm + " is : " + PackagingRate); 
				outputStreamWriter.write(PackagingRate);
				outputStreamWriter.flush();
				...
				System.out.println("Packaging Rate not send to server since client is stopped"); 
				return Response.status(Status.OK).entity("Packaging Rate not send to server since client is stopped").build();
			}
        } catch (Exception e) {
           e.printStackTrace();
           return Response.status(Status.INTERNAL_SERVER_ERROR).entity("Packaging rate update request failed !!").build();
        }
	}	
