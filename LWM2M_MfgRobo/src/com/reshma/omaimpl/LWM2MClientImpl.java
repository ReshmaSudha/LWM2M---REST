...
public class LWM2MClientImpl {
	...
	@POST
	@Produces("application/json")
	public Response updateProductionRate(@PathParam("EndPointClientName") String endPntCltNm, String ManufacturingRate) {
		OutputStreamWriter outputStreamWriter = null;
        InputStream inputStream = null;
        BufferedReader bufferedReader = null;
        try {
			...
			String status = new JSONObject(ManufacturingRate).getJSONObject("PRODRATEUPDATEREQUEST").getString("STATUS").toString();
			if(!status.equals("STOPPED")){
				...
				httpURLConnection.setRequestMethod("POST");
				outputStreamWriter = new OutputStreamWriter(httpURLConnection.getOutputStream());
				System.out.println("Update manufaturing rate request from client - "+ endPntCltNm + " is : " + ManufacturingRate); 
				outputStreamWriter.write(ManufacturingRate);
				outputStreamWriter.flush();
				...
				System.out.println("Manufacturing Rate not send to server since client is stopped"); 
				return Response.status(Status.OK).entity("Manufacturing Rate not send to server since client is stopped").build();
			}
        } catch (Exception e) {
           e.printStackTrace();
           return Response.status(Status.INTERNAL_SERVER_ERROR).entity("Manufacturing rate request failed !!").build();
        }
	}	
	...
	public Response getClientStatus() {
		...
		try {
			...
			if (resultSet.next()) {
				 ...
				 System.out.println("getStatusResponseJSON.toString()  is -------------------" + getStatusResponseJSON.toString());
            }
        } catch (Exception e) {
           e.printStackTrace();
