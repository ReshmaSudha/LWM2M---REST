...
public class LWM2MClientImpl {
	...
	@POST
	@Produces("application/json")
	public Response updateProductionRate(@PathParam("EndPointClientName") String endPntCltNm, String QARate) {
		OutputStreamWriter outputStreamWriter = null;
        InputStream inputStream = null;
        BufferedReader bufferedReader = null;
        try {
        	String status = new JSONObject(QARate).getJSONObject("PRODRATEUPDATEREQUEST").getString("STATUS").toString();
			if(!status.equals("STOPPED")){
				...
				httpURLConnection.setRequestMethod("POST");
				outputStreamWriter = new OutputStreamWriter(httpURLConnection.getOutputStream());
				System.out.println("Update QA rate request from client - "+ endPntCltNm + " is : " + QARate); 
				outputStreamWriter.write(QARate);
				outputStreamWriter.flush();
				...
				System.out.println("QA Rate not send to server since client is stopped"); 
				return Response.status(Status.OK).entity("QA Rate not send to server since client is stopped").build();
			}
